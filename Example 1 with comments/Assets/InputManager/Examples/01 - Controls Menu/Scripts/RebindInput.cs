...
namespace TeamUtility.IO.Examples
{
    ...
    public class RebindInput : MonoBehaviour, IPointerDownHandler
    {
        ...
        private void Awake()
        {
            ...
            //Adds method to ConfigurationDirty action! to handle ConfigurationDirty
        ...
        private void InitializeAxisConfig()
        {
            ...
            if (_axisConfig != null)
            {
                ...
                {
                    //Handles positive or negative key
                    if (_changePositiveKey)
                    {
                        // Handles alt key or regular key


                        //Makes text the key that's currently configured or sets it to empty ("").
                        if (_changeAltKey)
        ...
        //Unity function that can detect when input field is selected
        public void OnPointerDown(PointerEventData data)
        {
            StartCoroutine(StartInputScanDelayed());
        }

        //Here the actual scanning happens, with the ... in the image 
        private IEnumerator StartInputScanDelayed()
        {
            yield return null;

            //Upon start, it's not scanning so this is true by default!
            //Here the scanFlags are set.
        ...
        //Here the keybinding is actually changed
        private bool HandleKeyScan(ScanResult result)
        {
            //	When you return false you tell the InputManager that it should keep scanning for other keys
        ...
        /// <summary>
        /// As method name says reduces total amount of keys by a big amount
        /// to easify HandleKeyScan.
        /// </summary>
